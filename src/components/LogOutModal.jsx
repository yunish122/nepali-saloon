...
function LogOutModal({ name, onLogout, isLoggingOut = false }) {
    return (
        <div className="absolute right-0 top-10 w-42 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_16px_36px_rgba(15,23,42,0.14)]">
            ...
            <div className="border-t border-slate-200 px-1 py-1">
                ...
                <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition hover:bg-red-50/60 disabled:cursor-not-allowed disabled:opacity-60"
                >
                    <LogOut size={18} className="text-slate-500" strokeWidth={1.8} />
                    <span className="text-sm font-medium text-red-500">{isLoggingOut ? 'Logging out...' : 'Logout'}</span>
                </button>
            </div>
        </div>
    );
